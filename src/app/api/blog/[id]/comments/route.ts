  ...
  try {
    ...
    const rows = session
      ? await mysqlQuery<CommentRow>(
          "SELECT c.*, u.avatarUrl AS avatarUrl, IF(ucl.user_id IS NULL, 0, 1) AS likedByMe FROM comment c LEFT JOIN users u ON c.publisher_id = u.id LEFT JOIN user_comment_like ucl ON ucl.comment_id = c.id AND ucl.user_id = ? WHERE c.bn_id = ? AND c.category = 'blog' ORDER BY c.create_time ASC",
          [session.sub, id]
        )
      : await mysqlQuery<CommentRow>(
          "SELECT c.*, u.avatarUrl AS avatarUrl FROM comment c LEFT JOIN users u ON c.publisher_id = u.id WHERE c.bn_id = ? AND c.category = 'blog' ORDER BY c.create_time ASC",
          [id]
        );

    const totalRows = await mysqlQuery<{ total: number }>(
      "SELECT COUNT(*) AS total FROM comment WHERE bn_id = ? AND category = 'blog'",
      [id]
    );
